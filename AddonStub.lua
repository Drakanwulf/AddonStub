...
itself. Because AddonStub was derived from the ESO LibStub (r5) add-on, its structure should be familiar to those who know how
LibStub works; however, AddonStub does some things that LibStub does differently and it does some things that LibStub never did.
...
"manifest" table that has values extracted from your manifest file directives  and other startup data that the game  returns as
values from other API functions. This occurs whenever AddonStub loads an add-on successfully. The table is available to each
standalone add-on that uses AddonStub to bootstrap its loading process. You can use a "/tbug <youraddonname>.manifest" command
to view the table and its contents. "tbug" is a debugging utility developed by merTorchBug. You can get it from the ESOUI
website.
