...
AddonStub is a simple stub that has been designed and coded to provide a consistent method to load stand-alone add-ons including
itself. "Stand-alone" means that a AddonStub folder should not be embedded within any other add-on's folder. Use the ##DependsOn:
directive to instruct ESO to verify that an AddonStub has been loaded before ESO attempts to load your add-on. DO NOT embed an 
AddonStub folder within your add-on's folder!

As AddonStub loads your add-on, it builds a "manifest" table from the manifest file directives and other data that ESO provides
as return values from its "AddOnManager:GetAddOnInfo" API function. This occurs whenever AddonStub loads an add-on successfully.
The table is available to each Stand-Alone add-on that uses AddonStub to bootstrap its loading process. You can use a merTorchBug
"/tbug AddonStub.addon" or "/tbug <youraddonname>.addon" command to view the table and its content.

History:
	AddonStub is a descendant of the ESO "LibStub" library add-on which was derived from the World of Warcraft (WoW) LibStub
	library add-on. See http://www.wowace.com/wiki/LibStub for more information about the WoW LibStub. We thank everyone who
	helped us develop this add-on stub.
	...
	AddonStub was derived from the ESO LibStub (r5) add-on by Drakanwulf and Hawkeye1889.
